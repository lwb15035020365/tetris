...
const states = {
  ...
  start: () => {
    ...
    states.dispatchPoints(0);// 设置初始分数为0
    store.dispatch(actions.speedRun(state.get('speedStart'))); // 设置初始下落快慢等级
    const startLines = state.get('startLines');// 获取起始行
    const startMatrix = getStartMatrix(startLines);// 获取界面所有块(存在起始行，生成起始行)
    store.dispatch(actions.matrix(startMatrix));// 设置界面所有块
    store.dispatch(actions.moveBlock({ type: state.get('next') })); // 获取当前块
    store.dispatch(actions.nextBlock());// 获取下一个块
    states.auto();// 自动下落
  },
